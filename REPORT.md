...
**The seam is the load-bearing decision.** Above it nothing knows what a DOM is. An `Observation` is a flat list of `UiNode`, role, accessible name, value, container path, bounding box, and a `TargetDescriptor` says *"the textbox right of the cell reading Member Number"*. That vocabulary was chosen because it is the one description of a UI that exists everywhere: ARIA in a browser, `AXRole`/`AXTitle` on macOS, UIA on Windows. Artifacts describe intent, not implementation.

**One chokepoint.** `GuardedSurface` wraps the surface and enforces, in order: does this actor hold the control lease, does policy allow this, write both to evidence. The planner, the replay engine and the human operator each get their own over the *same* session; nothing else holds a reference to the raw surface. A guardrail the agent enforces on itself is a suggestion.
...
**Failure signatures** map a positively-identified broken screen to a real failure class. Without them an app rendering `CT-500 ORA-01722` reports as "expected text not found", true, useless, indistinguishable from a locator bug.
...
**Many tenants, one product.** *App profiles* hold what is true of the vendor product (session expiry, the interstitial, the `CT-500` page, the error banner every success condition excludes, its risk vocabulary) and the compiler merges them into every capability recorded against it, because rediscovering the same quirks across ~20 capabilities × hundreds of tenants gets them inconsistent thousands of times. Risk vocabulary must be per-product: "Remove" is destructive on a core banking screen and reversible on a shopping cart. *Specialisation is a patch, not a copy*: a tenant declares `extends` plus overrides, and `granite.member.read-savings-balance` declares no steps at all, because the schema permits an empty list only when `tenant.extends` is set. That patch is keyed on step ids, which makes their stability load-bearing. Ids derived from the model's own `intent` prose would not be stable: the same inquiry step reads `type-the-member-number-into-the` from one planner and `enter-member-number-to-look-up` from another, so every override would break the moment a base was re-recorded, which is precisely the situation the mechanism exists for. Ids therefore derive from the action and the target's identity, which describe the screen rather than the describer, and a scripted planner and a live model produce identical ones. So a base fix reaches every tenant that has not overridden that point, a tenant's deviation is a diff, and drift is localised. Policy combines conservatively (max risk, OR approval, AND idempotent), because Zod defaults make a naive merge silently downgrade a base's guardrails, and `lintSpecialisation` catches an incomplete override set at review time rather than at run time.
...
**Screenshots are redacted by the same policy as the text.** Images were the obvious hole: the log was scrubbed while the PNG beside it showed the tax ID in full. So `GuardedSurface.screenshot` asks the *same* `Redactor` which nodes it would rewrite, and hands their handles to the surface, which covers those regions during capture rather than editing the page first or the image afterwards. One policy over both media, so the two cannot drift apart. It is driven by content rather than by the artifact, so it behaves identically during discovery, during replay and while a human is driving, and it covers a tax ID nobody declared an output for. A surface that cannot mask must refuse rather than return an unmasked image: the macOS surface throws, because `screencapture` has no exclusion rectangles and compositing afterwards would mean the unmasked pixels existed first. `ask` is the one model-bound path with no scrubbing, and what protects credentials there is schema stripping.
...
**Now included, having been a cut.** A live model's judgement. The committed evidence is a `--live` run: all 17 entries as expected, all three discovery runs driven by `openai/gpt-oss-120b`, and everything downstream of them calling no model at all, which every replay result records as `plannerCalls: 0`. It was worth doing for what it found: a loop that could spin on turns which *succeed* without advancing (a model re-declared the same output eight times until the wall clock killed it), two rate-limit policies wrong in opposite directions (a token bucket wants retries, a spent daily allowance must not get them, and both arrive as HTTP 429), and the step-id coupling above. None were reachable from a scripted planner. A production desktop driver. Real co-browsing, out of scope per the brief. Persistence and scale: artifacts are files, interventions live in memory. Money as a float, which should be minor units in production. Profile merging is wholesale, so the sign-on capability advertises outcomes it cannot produce. Locator ladders derive from one observation, so a locator unique on this record's screen may be ambiguous on another's. And assisted LLM recovery on replay failure, deliberately: it is the feature most likely to erode the determinism guarantee by degrees, and the stability signal should be measured first.
